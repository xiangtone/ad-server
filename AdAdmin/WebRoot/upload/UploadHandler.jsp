	...
	if (!isMultipart) {
		...
		while (iter.hasNext()) {
			...
			if (item.isFormField()) {
				...
				session.setAttribute("FileUpload.Progress." + fileId,
						"-1");
				List<String> path = (List<String>) session
						.getAttribute("FileUpload.path" + fileId);
				if (path == null) {
					path = new ArrayList<String>();
				}
				path.add(uploadedFile.getPath());
				session.setAttribute("FileUpload.path" + fileId, new ArrayList(new HashSet(path)));
			}
		}
