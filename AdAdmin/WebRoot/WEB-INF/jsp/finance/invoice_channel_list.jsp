...
<script type="text/javascript">
	$(document).ready(function() {
		$("#invoiceReport").click(
		function() {
			$("#invoice").attr("action", "${pageContext.request.contextPath}/manage!invoiceChannelList.do").submit();
		});
	});
	function refresh() {
		$("#invoice").submit();
	...
	function sumit_num() {
		...
		if (checkedLength > 0) {
			...
			$.ajax({
				url:'${pageContext.request.contextPath}/manage!submitInvoiceChannelNum.do',
				type:'POST',
			...
			<div class="admin_right">
				<div class="content_right content_new">
					<fieldset class="margtop10 mar_bom10">
						<legend>发票管理</legend>
						<div id="search_bar">
							<form action="${pageContext.request.contextPath}/manage!invoiceChannelList.do" method="post" id="invoice" name="invoice">
								<table width="100%">
									<tr>
										<td>
											<table width="100%">
					...
					<input type="button" value="批量收票" onclick="sumit_num();" />
						...
						<table width="100%" cellpadding="0" cellspacing="1" id="tb" class="table_bod1 font_stat">
							...
							<c:forEach items="${list}" var="vo" varStatus="status">
								<tr style="text-align: center;">
									<td>
										<c:if test="${vo.invoice_status==null||vo.invoice_status==0}"><input id="invoice_id_${vo.id}" name="invoice_id" type="checkbox" value="${vo.id}"/>未收票</c:if>
										<c:if test="${vo.invoice_status==1}"><input id="invoice_id_${vo.id}" name="invoice_id" type="checkbox" value="${vo.id}" disabled="disabled"/>已收票</c:if>
									</td>
									<td>${vo.id}</td>
									<td>${vo.channel_id}</td>
