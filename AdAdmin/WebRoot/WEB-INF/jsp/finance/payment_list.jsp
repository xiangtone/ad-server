...
<script type="text/javascript">
	$(document).ready(function() {
		$("#paymentReport").click(
		function() {
			$("#payment").attr("action", "${pageContext.request.contextPath}/manage!financeIncomeAnswerlist.do").submit();
		});
	});
	function refresh() {
		$("#payment").submit();
	...
	function sumit_num() {
		...
		if (checkedLength > 0) {
			...
			$.ajax({
				url:'${pageContext.request.contextPath}/manage!submitPaymentNum.do',
				type:'POST',
			...
			<div class="admin_right">
				<div class="content_right content_new">
					<fieldset class="margtop10 mar_bom10">
						<legend>回款管理</legend>
						<div id="search_bar">
							<form action="${pageContext.request.contextPath}/manage!financeIncomeAnswerlist.do" method="post" id="payment" name="payment">
								<table width="100%">
									<tr>
										<td>
											<table width="100%">
					...
					<input type="button" value="批量回款" onclick="sumit_num();" />
						...
						<table width="100%" cellpadding="0" cellspacing="1" id="tb" class="table_bod1 font_stat">
							...
							<c:forEach items="${list}" var="vo" varStatus="status">
								<tr style="text-align: center;">
									<td>
										<c:if test="${vo.payment_status==null||vo.payment_status==0}"><input id="payment_id_${vo.id}" name="payment_id" type="checkbox" value="${vo.id}"/>未回款</c:if>
										<c:if test="${vo.payment_status==1}"><input id="payment_id_${vo.id}" name="payment_id" type="checkbox" value="${vo.id}" disabled="disabled"/>已回款</c:if>
									</td>
									<td>${vo.id}</td>
									...
									<td>${vo.name}</td>
									<td><c:if test="${vo.payment_status==1}">
									...
									<input type="text" id="payment_remark_${vo.id}" name="payment_remark" value="${vo.payment_remark}"/></c:if></td>
								</tr>
							</c:forEach>
