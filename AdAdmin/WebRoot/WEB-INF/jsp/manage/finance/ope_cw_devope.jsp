			...
			<div class="admin_right">
				<div class="content_right content_new">
					<fieldset class="search_fieldset">
						<legend>开发者提款运营审核</legend>
						<div id="search_bar">
							<form action="manage!listDMOperAudit.do" method="post" id="my_form">
								<div>
										...
										<td>
											<table width="100%">
												<tr>
													<td>
														<select name="keyword">
															<option value="DEV_ID" <c:if test="${bean.keyword == 'DEVELOPER_ID'}">selected="selected"</c:if>>开发者ID</option>
															<option value="DEV_EMAIL"  <c:if test="${bean.keyword == 'DEV_EMAIL'}">selected="selected"</c:if>>开发者</option>
						...
						<c:forEach items="${listDMOperAudit}" var="k">
							<tr>
								...
								<td style="text-align: center;">						
									<c:choose>
										<c:when test="${k.tax_status == 1}">
											正常
										</c:when>
										<c:when test="${k.tax_status == 2}">
											免税
										</c:when>									
									</c:choose>
								</td>
								...
								<td align="center"><c:choose>
										<c:when test="${k.status == 0}">
											<input type="button" value="通过"
												onclick="singleDealAuditInOper('${k.id}','1')" />
				...
				<c:if test="${not empty listDMOperAudit}">
					<div class="fr">
						...
						批量操作：<input	type="button" value="通过" onclick="batchDealAuditInOper('1')" />
						&nbsp;<input type="button" value="不通过" onclick="batchDealAuditInOper('-1');" />
					</div>
				</c:if>
			</div>
