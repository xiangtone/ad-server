...
<title>运营管理后台财务管理渠道提款财务审核</title>
...
<div class="main">
	<div class="content clearfix">
		<div class="content_right admin_right" style="overflow-x: scroll;">
			...
			<!--新增-->
			<form  method="post" id="listDMFinanceAudit" action="manage!financeChannekApplyList.do">
				...
				<table width="100%" border="1" cellpadding="1" cellspacing="0" bordercolor="#85caff">
					    <tr>
						      <td style="color: blue" bgcolor="#E8E8E8">
														<select name="keyword">
					...
					<td  colspan="4" bgcolor="#E8E8E8">
						<div style=" text-align: center;">
							<input name="" id=financeChaApplymoney value="查询" type="button" style="cursor: pointer;" />										
							<input name="" type="button" id="financeApplyChaDownload" value="导出Excel" style="cursor: pointer;" />
						</div>
					</td>
				</tr>
				</table>				
							...
							<td width="80px">
								<c:choose>
									<c:when test="${k.status == '1'}">
										...
										<input type="button" onclick="singleChannelFinance('${k.id}','2')" value="支付" style="background-color:transparent;border:0" />
										<input type="button" onclick="singleChannelFinance('${k.id}','-2')" value="拒付" style="background-color:transparent;border:0" />
									</c:when>
									<c:otherwise>
			...
			<c:if test="${not empty entiy}">
				<div class="fr"><input type="button" id="btn4" value="全选" />批量操作：<input type="button" value="支付" onclick="batchDealAuditChannelFinance('2')"/><input type="button" value="拒付" onclick="batchDealAuditChannelFinance('-2')" /></div>
			</c:if>
		</div>
	</div>
</div>
