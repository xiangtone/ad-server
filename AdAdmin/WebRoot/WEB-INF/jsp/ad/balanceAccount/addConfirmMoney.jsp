...
<body>
	<div class="main">
		<div class="content clearfix">
			<form id="updateAdAjustment" name="updateAdAjustment" action="update_confirm_money.do" method="post" >
				<div>
    			...
    			<div class="subblock_1" style="display:block;border: 0px solid #9dbaff;float: left;width: 866px;border-top: 0;height: 412px;">
					<table>
						<tr>
							<td valign="top">确认日期：</td>
							<td colspan="3">
								<input type="text" name="confirm_date"  class="Wdate" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'max_date\')}'});" value='<fmt:formatDate value="${vo.today}" type="date" dateStyle="long" pattern="yyyy-MM-dd HH:mm:ss"/>' /> 
							</td>
						</tr>
						<tr>
							<td>确认金额：</td>
							<td class="amount">
								<input type="text" name="confirm_money" value="${vo.price}" reg="(^[0-9]{1,8})+([.]\d{1,2})?$" onkeyup="if(isNaN(value))execCommand('undo')" onafterpaste="if(isNaN(value))execCommand('undo')" tip="请输入数字0-9，且小数点的最多为两位，总长度为10" maxlength="10"/>元
								<font color="red">*</font>	
							</td>
						</tr>
						<tr>
							<td>确认数：</td>
							<td class="amount">
								<input type="text" name="confirm_money" value="${vo.price}" reg="(^[0-9]{1,8})+([.]\d{1,2})?$" onkeyup="if(isNaN(value))execCommand('undo')" onafterpaste="if(isNaN(value))execCommand('undo')" tip="请输入数字0-9，且小数点的最多为两位，总长度为10" maxlength="10"/>个
								<font color="red">*</font>	
							</td>
						</tr>
					</table>
