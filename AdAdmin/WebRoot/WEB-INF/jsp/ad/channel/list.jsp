...
function adOnline(adID,status){
	var url = "${pageContext.request.contextPath}/manage!adChannelOnline.do";
	if(status == 1){
		alert("已经是上线状态");
		return;
	}
...
function adSuspend(adID,status){
	var url = "manage!adChannelOffline.do";
	if(status == 0){
		alert("已经是下线状态");
		return;
	}
...
function adOver(adID,status){
	var url = "manage!adChannelOver.do";
	if(status == 0){
		alert("已经是下线状态");
		return;
	}
...
function findUpdateAdv(adId){
		var url = "manage!adChannelDetail.do?adId="+adId;
		new $.dialog({
			title:'广告管理',
			page:url,
			width:575,
			...
			<div class="admin_right">
				<div class="content_right content_new">
					<fieldset class="search_fieldset">
						<legend>渠道广告管理 </legend>
						<div id="search_bar">
							<form action="manage!adChannelList.do" id="my_form" method="post">
								<table width="100%">
									<tr>
										<td>
						...
						<c:if test="${!empty list}">
							<c:forEach items="${list}" var="vo" varStatus="status">
								<tr>
									...
									<td>
										<escore:subStr len="20" value="${vo.cha_page}" />
									</td>
									<td>
										${vo.blanceMode}
									</td>
									...
									<td>
										<input type="button" value="修改" onclick="findUpdateAdv('${vo.adId}');" <c:if test="${vo.status==-5}">disabled</c:if>  />
										<input type="button" value="上线" onclick="adOnline('${vo.adId}','${vo.status}');" <c:if test="${(vo.status!=-1)&&(vo.status!=-20)}">disabled</c:if> />
										<input type="button" value="下线" onclick="adSuspend('${vo.adId}','${vo.status}');" <c:if test="${vo.status!=1}"> disabled</c:if> />
										<input type="button" value="投放结束" onclick="adOver('${vo.adId}','${vo.status}');" <c:if test="${vo.status!=-1}"> disabled</c:if> />
									</td>
									<td>
									</td>
								</tr>
