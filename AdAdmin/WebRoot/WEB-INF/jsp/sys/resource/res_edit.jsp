...
<div class="main">
	<div class="content clearfix">
		<div class="content_right admin_right">
			<form name="updateResource" action="manage!updateResource.do" id="my_form" method="post" enctype="multipart/form-data">
				<input name="id" id="id" type="hidden" maxlength="10" value="${entity.id }" />
				<table>
					<tr>
						<td width="100" valign="top">资源名称：</td>
						<td class="name">
							<input name="name" id="name" reg="^(?=.*?\S)[\s\S]{0,20}$" tip="资源名称不能为空" type="text" value="${entity.name }" maxlength="20" />*<font color="red">${errMsg_appName}</font>
						</td>
					</tr>
					<tr>
						<td width="100" valign="top">资源URL：</td>
						<td class="url">
							<input name="url" id="url" reg="^(?=.*?\S)[\s\S]{0,120}$" tip="url不能为空" type="text"	value="${entity.url }" maxlength="120" />* <font color="red">${errMsg_appName}</font>
						</td>
					</tr>
					<tr>
						<td width="100" valign="top">描述：</td>
						<td class="note">
							<input name="note" id="note" type="text" value="${entity.note}" maxlength="20" />
						</td>
					</tr>
					<tr>
						<td width="100" valign="top">上级ID：</td>
						<td class="parent_id">
							<input name="parent_id" id="parent_id" onkeyup="value=value.replace(/[^\d]/g,'')" type="text" reg="^(?=.*?\S)[\s\S]{0,40}$" tip="只能输入数字" value="${entity.parent_id }" maxlength="20" />
						</td>
					</tr>
					<tr>
						<td width="100" valign="top">排序字段 ：</td>
						<td class="order_num">
							<input name="order_num" id="order_num" onkeyup="value=value.replace(/[^\d]/g,'')" type="text" reg="^(?=.*?\S)[\s\S]{0,40}$" tip="只能输入数字" value="${entity.order_num}" maxlength="20" />
						</td>
					</tr>
					<tr>
						<td width="100" valign="top">报表配置id：</td>
						<td class="stat_report_id">
							<input name="stat_report_id" id="stat_report_id" onkeyup="value=value.replace(/[^\d]/g,'')" type="text" value="${entity.stat_report_id }" maxlength="20" />
						</td>
					</tr>
					<tr>
