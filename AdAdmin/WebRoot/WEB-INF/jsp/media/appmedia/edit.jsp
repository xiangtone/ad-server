...
<body>
	<div style="display: block;height: 500px;width: 900px;">
		<div style="display: block;float:left;width: 400px;height: 400px;">
			<form  method="post" id="my_form" action="${pageContext.request.contextPath}/updateAppMedia.do">
				<div>
				...
				<table width="100%">
					<tr>
						<td>
							<table width="100%">
								<tr>
									<td width="23%" valign="middle">名称：</td>
									<td width="77%" class="name" colspan="3">
										<input name="name" type="text" reg="^[\u4e00-\u9fa5\w\W]{2,10}$" value="${vo.name}" tip="真实姓名" maxlength="50" />
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="cost">
										<select name="area_type" id="area_type" style="width: 155px;" >
											...
											<option value="1" <c:if test="${vo.area_type ==1}"> selected="selected"</c:if> >微赢</option>
										</select>
										<font color="red">*</font>	
									</td>
								</tr>
									...
									<td class="cost">
										<select name="sys_user_id" style="width: 155px;">
											...
											<c:forEach items="${sysUserList}" var="entity">
												<option value="${entity.id}" <c:if test="${vo.sys_user_id eq entity.id}"> selected="selected" </c:if>>${entity.real_name}</option>
											</c:forEach>
										</select>
