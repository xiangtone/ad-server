...
function upateAmount(id){
	var url = "${pageContext.request.contextPath}/finance/getChannelById.do";
	$.ajax({
		url:url,
		type:'POST',
		data:'id='+id,
...
<body>
	<div style="display: block;height: 500px;width: 900px;">
		<div style="display: block;float:left;width: 400px;height: 400px;">
			<form  method="post" id="my_form" action="${pageContext.request.contextPath}/updateSalesman.do">
				<div>
				...
				<table width="100%">
					<tr>
						<td>
							<table width="100%">
								<tr>
									<td width="23%" valign="middle">销售名称：</td>
									<td width="77%" class="name" colspan="3">
										<input name="name" type="text" reg="^[\u4e00-\u9fa5\w\W]{2,10}$" value="${vo.name}" tip="销售名称" maxlength="50" />
									</td>
								</tr>
								<tr>
									<td valign="middle">电话：</td>
									<td colspan="3" class="mobile">
										<input type="text"  name="mobile" reg="^1[3,5,8,7]\d{9}$" value="${vo.mobile}" tip="请输入手机号 " maxlength="11"  />
									</td>
								</tr>
								<tr>
									<td valign="middle">QQ：</td>
									<td class="qq">
										<input type="text" name="qq" reg="[1-9][0-9]{5,}" tip="请输入QQ号" value="${vo.qq}" maxlength="20" />
									</td>
								</tr>
								<tr>
									<td valign="cost">区域：</td>
									<td class="cost">
										<select name="area_type" id="area_type" style="width: 155px;" >
											<option value="">请选择</option>
											<option value="0" <c:if test="${vo.area_type ==0}">selected="selected"</c:if> >平台</option>
											<option value="2" <c:if test="${vo.area_type ==2}"> selected="selected"</c:if> >华北</option>
											<option value="1" <c:if test="${vo.area_type ==1}"> selected="selected"</c:if> >华东</option>
											<option value="4" <c:if test="${vo.area_type ==4}"> selected="selected"</c:if> >华南</option>
										</select>
										<font color="red">*</font>	
									</td>
								</tr>
									...
									<td class="cost">
										<select name="sys_user_id" style="width: 155px;">
											...
											<c:forEach items="${sysUserList}" var="entity">
												<option value="${entity.id}" <c:if test="${vo.sys_user_id eq entity.id}"> selected="selected" </c:if>>${entity.realName}</option>
											</c:forEach>
										</select>
					...
					<tr><td><font color="red" style="font-size: 5px;">跟系统账户关联后,该销售人员才能正常查看数据</font></td></tr>
				</table>
			</form>
		</div>
	</div>
