...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
		      ...
		      {text: '昨天', dateStart: 'yesterday', dateEnd: 'yesterday' },
		      
		      {text: '本周(周一至昨天)', dateStart: function(){
		    	  												oToday=new Date();
		      ...
		      {text: '上周(周一至周日)', dateStart: function(){
										    	  		var oToday=new Date();
										    	  		oToday.setDate(oToday.getDate() - 7);
										    	  		currentDay=oToday.getDay();
										    	  		if(currentDay==0)currentDay=7;
										    	  		mondayTime=oToday.getTime()-(currentDay-1)*24*60*60*1000;
										    	  		return new Date(mondayTime);
		      ...
		      {text: '本月', dateStart: function(){ return Date.parse('yesterday').moveToFirstDayOfMonth();}, dateEnd: 'yesterday' },
		      ...
		      {text: '本年', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'yesterday'}
		], 
		//presetRanges: array of objects for each menu preset. 
		//Each obj must have text, dateStart, dateEnd. dateStart, dateEnd accept date.js string or a function which returns a date object
		presets: {
			specificDate: '选择一天', 
			allDatesBefore: '之前的一天', 
			allDatesAfter: '之后的一天', 
			dateRange: '自定义时间段'
		},
		...
		latestDate: Date.parse('yesterday'), //latest date allowed 
	...
	var datepickerOptions = {
		onSelect: function() {
				...
				//Modify by EMAR.leo at 2011-08-19 为页面表单中的时间隐藏域赋值
				$("#hid_beginDate").attr("value", rangeA);
				$("#hid_endDate").attr("value", rangeB);
			},
			defaultDate: +0
	};
