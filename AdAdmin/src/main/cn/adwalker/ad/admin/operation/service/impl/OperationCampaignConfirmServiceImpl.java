 ...
 * @date 2013-6-8
 */
@Service("campaignConfirmService")
public class OperationCampaignConfirmServiceImpl implements
		IOperationCampaignConfirmService {
	...
	public List<CampaignConfirmVo> findByPage(CampaignConfirmbean bean,
		...
		StringBuilder sb = new StringBuilder(
				" T_CAMPAIGN_CONFIRM a left join  V_CAMPAIGN t  on a.campaign_id=t.ID left join t_campaign_salesman s on t.salesman_id=s.id where a.STATUS>=1");
		if (bean != null) {
			...
			if (ObjectUtils.isNotEmpty(bean.getCompany_name())) {
				sb.append(" and t.company_name like '%");
				sb.append(bean.getCompany_name());
				sb.append("%'");
			}
		...
		return (List<CampaignConfirmVo>) campaignConfirmDao
				.findByPage(
						"a.*,t.campaign_name, t.charge_type,t.os,t.company_name,s.name ",
	...
	private CampaignSum findConfirmSum(CampaignConfirmForm confirmFrom)
		...
		List<CampaignSum> objects = (List<CampaignSum>) campaignConfirmDao
				.findAll(
						sb.toString(),
	...
	public void updateConfirm(CampaignConfirmEditForm form, SysUserVo manageUser)
		...
		entity = campaignConfirmDao.getCampaignConfirm(form.getId());
		if (entity != null) {
			...
			campaignConfirmDao.update(entity);
	...
	private CostChannelTmpSumVo public_platform(CampaignConfirm entity)
		...
		if (entity != null) {
			// 根据时间区间抓取数据存入中间表
			List<CostPlatFormVo> list = (List<CostPlatFormVo>) campaignConfirmDao
					.findAll(
	...
	public CampaignConfirmSumVo findSum(CampaignConfirmbean bean) {
		...
		return new CampaignConfirmSumVo(sum_income_money, sum_forecast_money,
				sum_forecast_amount);
	}
	...
	private List<CampaignConfirmSum> getSumList(CampaignConfirmbean bean) {
		...
		StringBuffer sb = new StringBuffer(
				"select sum(a.forecast_money) as sum_forecast_money,sum(a.income_money) as sum_income_money,sum(a.forecast_amount) as sum_forecast_amount from T_CAMPAIGN_CONFIRM a left join  V_CAMPAIGN t  on a.campaign_id=t.ID left join t_campaign_salesman s on t.salesman_id=s.id where a.STATUS>=1");
		if (bean != null) {
			if (ObjectUtils.isNotEmpty(bean.getMonth_stat_date())) {
				sb.append(" and a.month_stat_date>= '");
				sb.append(bean.getMonth_stat_date());
	...
	public void testPlatformCost(Long balance_id) throws Exception {
		List<SimpleBean<BigDecimal>> list = (List<SimpleBean<BigDecimal>>) campaignConfirmDao
				.findAll(
		...
		for (SimpleBean<BigDecimal> bean : list) {
			try {
				CampaignConfirm entity = campaignConfirmDao.get(bean.getObj()
	...
	public void testChannelCost(Long balance_id, Long media_id)
			throws Exception {
		try {
			CampaignConfirm entity = campaignConfirmDao.get(balance_id,
 ...
 */
	...
	* @see cn.adwalker.ad.admin.operation.service.IOperationCampaignConfirmService#updateInCome(java.util.List)
	...
	public int[] updateInCome(List<Object[]> objList,String[] idList)throws Exception {
		...
		iosAndroidDayNumDao.updateInCome(ars,1);
		return campaignConfirmDao.updateInCome(objList);
	...
	* @param object
	* @return
	* @see cn.adwalker.ad.admin.operation.service.IOperationCampaignConfirmService#updateInCome(java.util.List, java.lang.String[], java.lang.Object)
	 */
	@Override
	public int[] updateInCome(List<Object[]> objList, String[] idList,
		...
		iosAndroidDayNumDao.updateInCome(ars,3);
		return campaignConfirmDao.updateInCome(objList);
