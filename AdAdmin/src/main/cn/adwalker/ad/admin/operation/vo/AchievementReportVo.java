...
public class AchievementReportVo {
	// 主键
		...
		public Double getUllage() {
			ullage = 0d;
			if (forecast_amount != null && forecast_amount != 0
					&& ObjectUtils.isNotEmpty(income_amount)) {
				ullage = (forecast_amount.doubleValue() - income_amount
						.doubleValue()) / forecast_amount.doubleValue() * 100;
			}
			return ullage;
		}
		...
		public Double getCha_gross_profit_rate() {
			...
			if (ObjectUtils.isNotEmpty(channel_income_money)
				...
				d = (channel_income_money - channel_cost_money)
