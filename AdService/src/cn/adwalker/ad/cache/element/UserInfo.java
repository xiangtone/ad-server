...
public class UserInfo extends Data{
	...
	private String idfv;
	private List<UserAdRel> signAdRels=new ArrayList<UserAdRel>();
	private Map<String, UserScore> scoreMap = new HashMap<String, UserScore>();
	...
	public Map<String, UserScore> getScoreMap() {
		return scoreMap;
	}

	public void setScoreMap(Map<String, UserScore> scoreMap) {
	...
	public List<UserAdRel> getSignAdRels() {
		return signAdRels;
	}

	public void setSignAdRels(List<UserAdRel> signAdRels) {
		this.signAdRels = signAdRels;
	}
	...
	public NoticeInfo getNoticle(Long appId){
		UserScore us = getUserScore(appId);
		if(us!=null){
			String rate = us.getDownLoadTime()>=6?"100%":((us.getDownLoadTime()*100)/6)+"%";
			return new NoticeInfo(NumberUtil.getInteger(us.getScore(), 0),NumberUtil.getInteger(us.getTodayInteger(), 0) ,rate);
		}
		return new NoticeInfo(0,0 ,"0.00%");
	}
	public void putUserScore(UserScore us){
		dealNullMap();
		this.getScoreMap().put("user_add_score_"+us.getApp_id(), us);
	}
	public UserScore getUserScore(Long appId){
		dealNullMap();
		return this.getScoreMap().get("user_add_score_"+appId);
	}
	private void dealNullMap(){
		if(null==this.getScoreMap()){
			this.setScoreMap(new HashMap<String, UserScore>());
		}
	}
}
