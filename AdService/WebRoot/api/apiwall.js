...
function requestData(pageType, pageSize, params) {
	...
	xmlhttp.open("post", serverUrl + "api/adList.do" + "?app_key=" + params,true);
...
function scoreWall(data, pageSize) {
	...
	dataUndefined(data);
//	pageCount = data.page.wallPage.pageCount;
	pageCount =1;
	var list = data.list;
	if (list.length > 0) {
		for ( var i in list) {
...
function drawItem2(index, item, catagory) {
	...
	boxDiv.addEventListener('click', function() {
		downloadFile(item.ad_type, item.ad_url, item.app_url, item.id, index,
				item.apple_id, "6",item.appid,item.is_url_params,item.url_params);
	}, false);
	// 加入到boxLM当中
...
function drawItem(index, item, catagory) {
	...
	boxMDiv.innerHTML = "<div id='title" + index + "' class='title'>"
			+ item.ad_name + "</div><div id='catagory" + index
			+ "' class='catagory'>" + item.slogan
	...
	downMiddle.innerHTML = "<span class='downjinbi'>" +item.score_unit
			+ "</span>";
	var downBottom = document.createElement("div");
