$(function() {
	$("#type").change(function() {
		var id = $(this).val();
		var url="provinceCitySort.action?id="+id+"&t="+new Date().getTime();
		if(id !=""&& id != 0){
	...
	$("#platform").change(function() {
		...
		var type = $("#type").val();
		var url="getCategory.action?type="+type+"&platform="+platform;
...
function getCategoryById(categoryId){
	var url="getCityId.action?categoryId="+categoryId+"&t="+new Date().getTime();
	var type;
	var platform;
	if(categoryId!=""){
		 $.ajax({   
...
function getPlatform(type,platform){
	var url="provinceCitySort.action?id="+type+"&selectId="+platform;
	if(type !="" && type!= 0){
		$.get(url, function(msg){
			$("#platform option").each(function() {                              
				$(this).remove();//移除原有项                          
