			...
			<div id="pad-wrapper" class="users-list">
				<div class="row header">
					...
					<div class="col-md-10 col-sm-12 col-xs-12 pull-right">
						<a href="toAddApplication1.action" class="btn-glow success pull-right"><span>&#43;</span>添加应用
						</a>
					</div>
				</div>
				<div class="table-products section">
					<div class="row filter-block">
						<form action="reportActualTime.action" id="reportActualTime" name="reportActualTime" method="post">
							...
							<button class="btn-flat new-product" onclick="$('#reportActualTime').submit();">查询</button>
						</form>
					</div>
					<div class="col-md-12">
						<h4 class="clearfix">
							图表
							<div class="btn-group">
								<button class="glow left active" value=" " onclick="bthScore('')">全部</button>
								<button class="glow middle" value="0" <c:if test="${bean.type_id ==0}"> style="color: #333333;" </c:if> onclick="bthScore(0)">积分墙</button>
								<button class="glow middle" value="1" <c:if test="${bean.type_id ==1}"> style="color: #333333;" </c:if> onclick="bthScore(1)">推荐墙</button>
								<button class="glow right" value="5" <c:if test="${bean.type_id ==5}"> style="color: #333333;" </c:if> onclick="bthScore(5)">插屏</button>
							</div>
						</h4>
					</div>
					<div class="col-md-12">
								...
								<tbody>
									...
									<c:if test="${!empty list}">
										...
										<c:forEach items="${list}" var="aml" varStatus="status">
											<tr>
												<td>${bean.end_date}-${aml.static_hour}</td>
												<td>${aml.pv}</td>
