...
/**
 * 读取status.properties配置文件 并内置线程2分钟更新一次
 * 
 * @author gary wujiapen
 * 
 */
public class StatusConfigUtil {

	private static final Logger log = LoggerFactory.getLogger(StatusConfigUtil.class);
	...
	static {
		try {
			java.net.URL url = StatusConfigUtil.class.getResource("");
			int index = url.getPath().indexOf("/WEB-INF/"); // 取属性文件的绝对路径
			String pathPrefix = url.getPath().substring(0, index + 9).replace("%20", " "); // 替换掉空格
			propFilePath = pathPrefix + "status.properties";

			// 加载属性文件
			FileInputStream fis = new FileInputStream(propFilePath);
			bundle = new PropertyResourceBundle(fis);
			// 启动属性文件监视线程
			monitor = new StatusConfigUtil().new PropertyMonitor();
			monitor.start();
		} catch (Exception e) {
	...
	public static String getString(String key) {
		try {
			String result = bundle.getString("APP_"+key);
			return result;
