...
public class AlanXUploadBodyTag implements Tag {
	...
	public int doEndTag() throws JspException {
		try {
			...
			Logger.getLogger(AlanXUploadBodyTag.class.getName()).log(
					Level.SEVERE, "初始化组件失败", ex);
			try {
				pageContext
						.getOut()
						.write(
								"初始化组件失败，有任何问题，请在<a href=\"http://www.alanx.cn\" target=\"new\">http://www.alanx.cn</a>上留言");
			} catch (IOException e) {
				Logger.getLogger(AlanXUploadBodyTag.class.getName()).log(
						Level.SEVERE, "初始化组件失败", ex);
			}
