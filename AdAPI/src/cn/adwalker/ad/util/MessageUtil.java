...
public class MessageUtil {
	...
	static {
		try {
			java.net.URL url = MessageUtil.class.getResource("");
			int index = url.getPath().indexOf("/WEB-INF/"); // 取属性文件的绝对路径
			String pathPrefix = url.getPath().substring(0, index + 9).replace("%20", " "); // 替换掉空格
			propFilePath = pathPrefix + "message.properties";

			// 加载属性文件
			FileInputStream fis = new FileInputStream(propFilePath);
			bundle = new PropertyResourceBundle(fis);
			// 启动属性文件监视线程
			monitor = new MessageUtil().new PropertyMonitor();
			monitor.start();
		} catch (Exception e) {
