 ...
 * 桔子渠道神仙道广告对接
 */
public class JuZhiPartherShenXianDaoCheck extends PartherCheck {
    private static final Logger logger = Logger.getLogger(JuZhiPartherShenXianDaoCheck.class);
    private CampaignConfig config=null;
    private IosActionLog action=null;
    private static final String skey="123456789"; //"c86be9aca16829a2467f7472f9b89c79";
	...
	public JuZhiPartherShenXianDaoCheck getInstance(){
		return new JuZhiPartherShenXianDaoCheck();
	}
	@Override
	...
	private Map<String, String> getParamter(ChannelBean data){
		...
		map.put("market_id", "yijifen");
		map.put("mac", data.isIos6()?StringUtil.dealNull(data.getDeviceid()).replaceAll(":", ""):StringUtil.dealNull(data.getIDFA()).toUpperCase());
		map.put("drkey", data.getSource());//保存渠道信息
		String mac = data.isIos6()?data.getDeviceid():data.getIDFA();
	    mac = StringUtil.dealNull(mac).replaceAll("-","").replace(":", "");
		map.put("target", "0");
		return map;
	}
	...
	boolean updateComriamActive(YjfBean bean) {
        ...
        if(obj!=null){
           ...
           String imei =  obj.containsKey("appid")?obj.getString("mac"):null;//激活标识
           String tradeno =  obj.containsKey("udata")?obj.getString("udata"):null;
           String pridata=  obj.containsKey("tradeno")?obj.getString("tradeno"):null;//保存渠道信息
