...
/**
 * <p>mac 和 idfa 公用一个参数服务 2013-12-27</p>
 * @author jief
 *
 */
public class OneDeviceidServiceImpl  implements ISendClickService{
private static final Logger log=Logger.getLogger(OneDeviceidServiceImpl.class);
	
	public static OneDeviceidServiceImpl oneDeviceidService=null;
	private OneDeviceidServiceImpl(){}
	public static OneDeviceidServiceImpl getInstance(){
	   if(oneDeviceidService==null){
		   synchronized (RawMacServiceImpl.class) {
			   if(oneDeviceidService==null){
				   oneDeviceidService=new OneDeviceidServiceImpl();
			   }
		   }
		}
		return oneDeviceidService;
	}
	@Override
...
//				params.append(ios.getUrl()).append("?");
				...
				}else if(StringUtils.isNotBlank(vo.getDeviceId())){
					params.append(ios.getDeviceid_para()).append("=").append(vo.getDeviceId().toLowerCase());
				}
