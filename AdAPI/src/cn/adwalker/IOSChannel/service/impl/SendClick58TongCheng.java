...
/**
 * <p>58同城,mac和idfa公用一个参数，mac 优先并且带":"</p>
 * @author jief
 *
 */
public class SendClick58TongCheng implements ISendClickService{
private static final Logger log=Logger.getLogger(SendClick58TongCheng.class);
	
	public static SendClick58TongCheng oneDeviceidMacFirstService=null;
	private SendClick58TongCheng(){}
	public static SendClick58TongCheng getInstance(){
	   if(oneDeviceidMacFirstService==null){
		   synchronized (RawMacServiceImpl.class) {
			   if(oneDeviceidMacFirstService==null){
				   oneDeviceidMacFirstService=new SendClick58TongCheng();
			   }
		   }
		}
...
//				params.append(ios.getUrl()).append("?");
				if(StringUtils.isNotBlank(vo.getDeviceId())){
					params.append(ios.getDeviceid_para()).append("=").append(StringUtil.formatMac(vo.getDeviceId()));
